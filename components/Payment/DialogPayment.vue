...
export default {
  ...
  methods: {
    ...
    actionDocument(action) {
      ...
      switch (action) {
        ...
        case 'PO':
          ...
          setTimeout(() => {
            this.actionName = 'Save'
            this.$refs.formPayment.changeValue('type', action)
            this.$refs.formPayment.changeValue('parent_id', document)
          }, 300)
          break
        case 'C':
          this.$refs.formPayment.changeValue('status', 'cancel')
          this.store()
          break
        case 'sendEmail':
    ...
    save(action) {
      ...
      const data = {
        ...this.$refs.formPayment.returnData(docId),
        action,
      }
      const vm = this
      this.loading = true
